    ...
    async function loadCustomerData() {
      try {
        ...
        if (currentCustomer.id) {
          ...
          try {
            const walletResponse = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:5000").replace(/\/+$/, "")}/wallet/${currentCustomer.id}`);
        ...
        try {
          ...
          if (savedCustomer.id) {
            ...
            try {
              const walletResponse = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:5000").replace(/\/+$/, "")}/wallet/${savedCustomer.id}`);
              const walletData = await walletResponse.json();
              if (mounted && walletResponse.ok) setWalletBalance(Number(walletData?.balance) || 0);
            } catch (e) { if (mounted) setWalletBalance(0); }
          }
